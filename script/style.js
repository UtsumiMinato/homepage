...
let backgroundSettingResetBtn = document.querySelector(
    '#background-setting-reset-btn'
);
...
function valueChange() {
    ...
    switch (nowLanguage) {
        case 'chineseTraditional':
            ...
            break;
        case 'japanese':
            serachFont.placeholder = `${
                document.querySelector('#search-engine-select').value
            } で検索、または URL を入力します`;
            console.log(nowLanguage);
            break;
        case 'englishUs':
...
let linkMenuSettingTitleFont = document.querySelector(
    '#link-menu-setting-title'
);
...
let backgroundResetBtn = document.querySelector(
    '#background-setting-reset-btn'
);
...
function chineseTraditionalSet() {
    ...
    linkMenuSettingTitleFont.innerText =
        chineseTraditional.linkMenuSettingTitle;
...
function changeFileName() {
    document.querySelector('#background-name').innerText =
        'File：' + backgroundFile.value.split('\\')[2];
}
...
function setSite(num) {
    document.querySelector(`#site-name-show-${num}`).innerText =
        document.querySelector(`#site-name-${num}`).value;
    document.querySelector(`#site-url-${num}`).href = document.querySelector(
        `#site-url-input-${num}`
    ).value;
    setCookie(
        `sitename${num}`,
    ...
    setCookie(
        `siteurl${num}`,
        `${document.querySelector(`#site-url-input-${num}`).value}`
    );
}
...
async function config() {
    ...
    for (i = 1; i < 5; i++) {
        if (getCookie(`sitename${i}`) != undefined) {
            //設定快捷輸入框語言
            document.querySelector(`#site-name-show-${i}`).innerText =
                getCookie(`sitename${i}`);
            document.querySelector(`#site-url-${i}`).href = getCookie(
                `siteurl${i}`
            );
        }
    }
    ...
    if (getCookie('backgroundBlur')) {
        //設定背景透明度
        backgroundBlur.style.backdropFilter = `Blur(${getCookie(
            'backgroundBlur'
        )}px)`;
